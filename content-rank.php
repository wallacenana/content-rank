...
if (!defined('CONTENT_RANK_GENERATOR_UPDATE_MANIFEST_URL')) {
    define('CONTENT_RANK_GENERATOR_UPDATE_MANIFEST_URL', 'https://raw.githubusercontent.com/wallacenana/content-rank/main/update.json?v=1.9.107');
}
...
if (!class_exists('Content_Rank_Generator')) {
    ...
    final class Content_Rank_Generator
    {
        ...
        public static function normalize_tmdb_thumbnail_layout($value)
        {
            ...
            $allowed = array('rotate', 'standard', 'skew', 'skew_alt', 'center_focus', 'spotlight', 'blur_background');
        ...
        public static function create_post_from_generator_item($generator, $item, $prepared_article = null, $existing_post_id = 0)
        {
            ...
            $tmdb_content_image_sections = ($use_tmdb_thumbnail && class_exists('Content_Rank_TMDB') && !empty($item['tmdb_movies']) && is_array($item['tmdb_movies']))
                ...
                : array();
            error_log('[Content Rank][thumbnail] secoes TMDB para conteudo=' . count($tmdb_content_image_sections));
            ...
            try {
            if (!empty($tmdb_content_image_sections) && !empty($article['content_html'])) {
                $images_before = preg_match_all('/<img\b/i', (string) $article['content_html']);
                ...
                $images_after = preg_match_all('/<img\b/i', (string) $article['content_html']);
                error_log('[Content Rank][thumbnail] imagens TMDB no conteudo antes=' . intval($images_before) . ' depois=' . intval($images_after));
            }
